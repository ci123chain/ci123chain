...
func (k Keeper) HandleValidatorSignature(ctx sdk.Context, addr sdk.AccAddress, power int64, signed bool) {
	logger := k.Logger(ctx)
	...
	if missed {
		ctx.EventManager().EmitEvent(
			sdk.NewEvent(
				...
				sdk.NewAttribute([]byte(types.AttributeKeyMissedBlocks), []byte(fmt.Sprintf("%d", signInfo.MissedBlocksCounter))),
				sdk.NewAttribute([]byte(types.AttributeKeyHeight), []byte(fmt.Sprintf("%d", height))),
			),
		)
		...
		if found && !validator.IsJailed() {
			...
			ctx.EventManager().EmitEvent(
				sdk.NewEvent(
					...
					sdk.NewAttribute([]byte(types.AttributeKeyReason), []byte(types.AttributeValueMissingSignature)),
					sdk.NewAttribute([]byte(types.AttributeKeyJailed), []byte(consAddr.String())),
				),
			)
