...
import (
	...
	st "github.com/ci123chain/ci123chain/pkg/abci/store"
...
func (k Keeper) SigningInfo(c context.Context, req *types.QuerySigningInfoRequest) (*types.QuerySigningInfoResponse, error) {
	...
	consAddr, err := sdk.AccAddressFromBech32(req.ConsAddress)
	if err != nil {
		return nil, err
	}
...
func (k Keeper) SigningInfos(c context.Context, req *types.QuerySigningInfosRequest) (*types.QuerySigningInfosResponse, error) {
	...
	sigInfoStore := st.NewPrefixStore(store, types.ValidatorSigningInfoKeyPrefix)
	pageRes, err := pagination.Paginate(sigInfoStore, req.Pagination, func(key []byte, value []byte) error {
