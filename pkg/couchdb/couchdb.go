...
func (db *Database) Compact() (resp string, e error) {
	...
	dbResponse, err := db.connection.request("" +
		"", url, strings.NewReader(emtpyBody), headers, db.auth)
...
func (db *Database) ReadRange(startKey, endKey string) (*RangeQueryResponse, error) {
	...
	parameters := url.Values{}
	parameters.Set("include_docs", "true")
	parameters.Add("inclusive_end", "false") // endkey should be exclusive to be consistent with goleveldb
	parameters.Add("startkey", "\""+startKey+"\"")
	parameters.Add("endkey", "\""+endKey+"\"")
	url, err := buildParamUrl(parameters, db.dbName, "_all_docs")
	if err != nil {
		return nil, err
	}
