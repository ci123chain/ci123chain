...
func ResponseCheckTx(err error, gw, gu uint64, debug bool, events []abci.Event) abci.ResponseCheckTx {
...
// from the given error and gas values.
func ResponseDeliverTx(err error, gw, gu uint64, debug bool, events []abci.Event) abci.ResponseDeliverTx {
	space, code, log := ABCIInfo(err, debug)
