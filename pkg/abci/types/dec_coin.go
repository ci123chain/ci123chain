...
func (coin DecCoin) Add(coinB DecCoin) DecCoin {
	if coin.Denom != coinB.Denom && coin.Denom != "" {
		panic(fmt.Sprintf("coin denom different: %v %v\n", coin.Denom, coinB.Denom))
	}
	if coinB.Amount.IsZero(){
		return coin
	}
	if coin.Amount.IsZero() {
		return coinB
	}
	return DecCoin{coinB.Denom, coin.Amount.Add(coinB.Amount)}
}
...
func (coin DecCoin) MulDec(d Dec) DecCoin{
	...
	res := DecCoin{Amount:coin.Amount.Mul(d), Denom:coin.Denom}
	return res
}
...
func (coin DecCoin) TruncateDecimal() (Coin, DecCoin) {
	...
	return NewCoin(DefaultBondDenom ,truncated), NewDecCoinFromDec(coin.Denom, change)
}
...
func (coin DecCoin) QuoDecTruncate(d Dec) DecCoin {
	...
	res := DecCoin{Amount:coin.Amount.QuoTruncate(d), Denom:coin.Denom}
