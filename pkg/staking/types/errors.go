...
var (
	ErrCommissionUpdateTime = sdkerrors.Register(StakingCodespace, 17, "new rate cannot be changed more than once within 24 hours")
	ErrInvalidRequest = sdkerrors.Register(StakingCodespace, 18, "invalid request")
	ErrCommissionNegative = sdkerrors.Register(StakingCodespace, 19, "commission must be positive" )
	ErrCommissionHuge = sdkerrors.Register(StakingCodespace, 20, "commission cannot be more than 100%")
	ErrCommissionGTMaxRate = sdkerrors.Register(StakingCodespace, 21, "commission change rate cannot be more than the max rate")
	ErrCommissionChangeRateNegative = sdkerrors.Register(StakingCodespace, 22, "commission change rate must be positive")
	ErrCommissionChangeRateGTMaxRate = sdkerrors.Register(StakingCodespace, 23, "commission change rate cannot be more than the max rate")
	ErrDelegatorShareExRateInvalid = sdkerrors.Register(StakingCodespace, 24, "cannot delegate to validators with invalid (zero) ex-rate")
	ErrInsufficientShares = sdkerrors.Register(StakingCodespace, 25, "insufficient delegation shares")
	ErrUnknowTokenSource = sdkerrors.Register(StakingCodespace, 26, "unknown token source bond status")
	ErrInvalidValidatorStatus = sdkerrors.Register(StakingCodespace, 27, "invalid validator status")
	ErrBondedTokendFailed = sdkerrors.Register(StakingCodespace, 28, "notBondedTokensToBonded failed")
	ErrBondedTokensToNoBondedFailed = sdkerrors.Register(StakingCodespace, 29, "BondedTokensToBonded failed")
	ErrNoDelegation = sdkerrors.Register(StakingCodespace, 30, "no delegation")
	ErrBadSharesAmount = sdkerrors.Register(StakingCodespace, 31, "invalid shares amount")
	ErrSelfRedelegation = sdkerrors.Register(StakingCodespace, 32, "cannot redelegate to the same validator")
	ErrNoValidatorFound = sdkerrors.Register(StakingCodespace, 33, "no validator found")
	ErrNotEnoughDelegationShares = sdkerrors.Register(StakingCodespace, 34, "not enough delegation shares")
	ErrNoDelegatorForAddress = sdkerrors.Register(StakingCodespace, 35, "delegator does not contain delegation")
	ErrMaxUnbondingDelegationEntries = sdkerrors.Register(StakingCodespace, 36, "too many unbonding delegation entries for (delegator, validator) tuple")
	ErrTinyRedelegationAmount = sdkerrors.Register(StakingCodespace, 37, "too few tokens to redelegate (truncates to zero tokens)")
	ErrMaxRedelegationEntries = sdkerrors.Register(StakingCodespace, 38, "too many redelegation entries for (delegator, src-validator, dst-validator) tuple")
	ErrTransitiveRedelegation = sdkerrors.Register(StakingCodespace, 39, "redelegation to this validator already in progress; first redelegation to this validator must complete before next redelegation")
	ErrBadRedelegationDst = sdkerrors.Register(StakingCodespace, 40, "redelegation destination validator not found")
)
...
func ErrNoExpectedValidator(err error) error {
	return sdkerrors.Wrap(sdkerrors.ErrParams, fmt.Sprintf("unexpected validator: %v", err.Error()))
}
//
