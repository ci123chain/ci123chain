...
func ConvertProofs(tmProof *crypto.ProofOps) (MerkleProof, error) {
	if tmProof == nil {
		return MerkleProof{}, sdkerrors.New("tendermint proof is nil")
	}
	...
	for i, op := range tmProof.Ops {
		...
		if err != nil || p.Proof == nil {
			return MerkleProof{}, sdkerrors.Errorf("could not unmarshal proof op into CommitmentProof at index %d: %v", i, err)
		}
		proofs[i] = &p
	}
	return MerkleProof{
