...
func (q Keeper) ClientStates(c context.Context, req *types.QueryClientStatesRequest) (*types.QueryClientStatesResponse, error) {
	...
	store := prefix.NewPrefixStore(ctx.KVStore(q.storeKey), host.KeyClientStorePrefix)
	...
	//res := &types.QueryClientStatesResponse{
	//	ClientStates: clientStates,
	//	Pagination:   pageRes,
	//}
	////by, err := res.Marshal()
	////fmt.Println(string(by))
	//by1 := q.binCdc.MustMarshalJSON(res)
	//fmt.Println("===================")
...
func (q Keeper) ConsensusStates(c context.Context, req *types.QueryConsensusStatesRequest) (*types.QueryConsensusStatesResponse, error) {
	...
	store := prefix.NewPrefixStore(ctx.KVStore(q.storeKey), host.FullClientKey(req.ClientId, []byte(fmt.Sprintf("%s/", host.KeyConsensusStatePrefix))))
