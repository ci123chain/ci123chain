...
This lets the lite clients keep track of arbitrary changes to the validator set,
...
a lite clients can sync up with every block header to verify each validator set
...
lite clients syncing is optimized to use intelligent bisection and
...
The suggested use for local light clients is clients.NewHTTPProvider(...) for
...
static validator set, you store it locally upon initialization of the clients,
...
the clients, as that is the root of all trust.
