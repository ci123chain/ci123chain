...
func (k *Keeper) InitializeAndSeal(ctx sdk.Context) {
	...
	prefixStore := store.NewPrefixStore(ctx.KVStore(k.storeKey), types.KeyPrefixIndexCapability)
...
func (sk ScopedKeeper) ClaimCapability(ctx sdk.Context, cap *types.Capability, name string) error {
	if cap == nil {
		return errors.New("cannot claim nil capability")
	}
	if strings.TrimSpace(name) == "" {
...
func (sk ScopedKeeper) ReleaseCapability(ctx sdk.Context, cap *types.Capability) error {
	if cap == nil {
		return errors.New("cannot release nil capability")
	}
	name := sk.GetCapabilityName(ctx, cap)
	if len(name) == 0 {
		return errors.Errorf("capability not owned by module: %s", sk.module)
	}
	...
	prefixStore := store.NewPrefixStore(ctx.KVStore(sk.storeKey), types.KeyPrefixIndexCapability)
