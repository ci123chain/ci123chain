...
func TestSortition(t1 *testing.T) {
	...
	for i := 0; i < rounds; i++ {
		...
		// nominate validators
		for k := 0; k < N; k++ {
			_, j := Sortition(seed, attendees[k].pubkey, t, s, int64(attendees[k].power), int64(W))
			statistics[k] += int(j)
		}
	}
