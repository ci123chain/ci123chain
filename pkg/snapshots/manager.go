...
import (
	"bytes"
	"crypto/sha256"
	"github.com/ci123chain/ci123chain/pkg/snapshots/types"
	terrors "github.com/ci123chain/ci123chain/pkg/snapshots/types"
...
func (m *Manager) beginLocked(op operation) error {
	if op == opNone {
		return sdkerrors.Wrap(terrors.ErrLogic, "can't begin a none operation")
	}
	if m.operation != opNone {
		return sdkerrors.Wrapf(terrors.ErrConflict, "a %v operation is in progress", m.operation)
	}
	m.operation = op
	return nil
}
...
func (m *Manager) Create(height uint64) (*types.Snapshot, error) {
	if m == nil {
		return nil, sdkerrors.Wrap(terrors.ErrLogic, "no snapshot store configured")
	}
	...
	if latest != nil && latest.Height >= height {
		return nil, sdkerrors.Wrapf(terrors.ErrConflict,
...
func (m *Manager) Restore(snapshot types.Snapshot) error {
	...
	case done := <-chDone:
		...
		return sdkerrors.Wrap(terrors.ErrLogic, "restore ended unexpectedly")
	case <-chReady:
	}
...
func (m *Manager) RestoreChunk(chunk []byte) (bool, error) {
	...
	if m.operation != opRestore {
		return false, sdkerrors.Wrap(terrors.ErrLogic, "no restore operation in progress")
	}

	if int(m.restoreChunkIndex) >= len(m.restoreChunkHashes) {
		return false, sdkerrors.Wrap(terrors.ErrLogic, "received unexpected chunk")
	}
	...
	case done := <-m.chRestoreDone:
		...
		return false, sdkerrors.Wrap(terrors.ErrLogic, "restore ended unexpectedly")
	default:
	}
	...
	if int(m.restoreChunkIndex) >= len(m.restoreChunkHashes) {
		...
		if !done.complete {
			return false, sdkerrors.Wrap(terrors.ErrLogic, "restore ended prematurely")
		}
		return true, nil
	}
	return false, nil
