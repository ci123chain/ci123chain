...
func NewStore(db db.DB, dir string) (*Store, error) {
	if dir == "" {
		return nil, sdkerrors.Wrap(terrors.ErrLogic, "snapshot directory not given")
	}
...
func (s *Store) Delete(height uint64, format uint32) error {
	...
	s.mtx.Unlock()
	if saving {
		return sdkerrors.Wrapf(terrors.ErrConflict,
...
func (s *Store) Save(
	...
	if height == 0 {
		return nil, sdkerrors.Wrap(terrors.ErrLogic, "snapshot height cannot be 0")
	}
...
func decodeKey(k []byte) (uint64, uint32, error) {
	if len(k) != 13 {
		return 0, 0, sdkerrors.Wrapf(terrors.ErrLogic, "invalid snapshot key with length %v", len(k))
	}
	if k[0] != keyPrefixSnapshot {
		return 0, 0, sdkerrors.Wrapf(terrors.ErrLogic, "invalid snapshot key prefix %x", k[0])
	}
