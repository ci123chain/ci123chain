...
if [ ! -f $CI_HOME/config/genesis.json ]; then
    if [ -z $CI_MASTER_DOMAIN ] && [ -z $CI_EXPORT ]; then
        ...
        if [ $CI_VALIDATOR_ADDRESS ]; then
          ...
          /opt/cid-linux add-genesis-validator $CI_VALIDATOR_ADDRESS 10000000000000000000 $CI_PUBKEY 1 40 5 --home=$CI_HOME --moniker=$CI_NODE_NAME
        else
          /opt/cid-linux add-genesis-validator 0x3F43E75Aaba2c2fD6E227C10C6E7DC125A93DE3c 10000000000000000000 $CI_PUBKEY 1 40 5 --home=$CI_HOME --moniker=$CI_NODE_NAME
        fi
...
echo "---Start upgrade---"
/opt/upgrade --home=$CI_HOME
if [ $? -eq 0 ]; then
    echo "---Start cid---"
    exec $CI_HOME/bin/cid-linux start --laddr=tcp://0.0.0.0:80 --home=$CI_HOME
fi
