...
server-linux:
	GOPROXY=$(PROXY) CGO_ENABLED=0 GOOS=linux GOARCH=amd64 $(GO_BUILD_CMD) GOSUMDB=off -o ./docker/node/build/cid-linux ./cmd/cid
client-linux: 
	GOPROXY=$(PROXY) CGO_ENABLED=0 GOOS=linux GOARCH=amd64 $(GO_BUILD_CMD) GOSUMDB=off -o ./docker/node/build/cli-linux ./cmd/cicli
