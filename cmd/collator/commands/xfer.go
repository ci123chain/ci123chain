...
`, appName, appName, appName, appName)),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			var dstAddr = args[3]
			if sdk.HexToAddress(dstAddr).Empty() {
				return errors.New("address params invalid")
			}
...
func setPathsFromArgs(src, dst *collactor.Chain, name string) (*collactor.Path, error) {
